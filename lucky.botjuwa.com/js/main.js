...
(function ($) {
        ...
        getConversations(onSuccess = false, exclude_id) {
            if (this.id) {
                SBF.ajax({
                    ...
                    user_id: this.id,
                    department: main_chating_department,
                    ...
                    if (!SBF.errorValidation(response)) {
                        let conversations = [];
                        for (var i = 0; i < response.length; i++) {
                            if (SBChat.isConversationAllowed(response[i].source) && (response[i].conversation_status_code != 3 || ((!tickets && !CHAT_SETTINGS.close_chat) || (tickets && !CHAT_SETTINGS.tickets_close)))) {
        ...
        getConversationsCode(conversations = false) {
            ...
            for (var i = 0; i < conversations.length; i++) {
                if (conversations[i] instanceof SBConversation) {
                    if (!admin && conversations[i].status_code == 3 && ((!tickets && CHAT_SETTINGS.close_chat) || (tickets && CHAT_SETTINGS.tickets_close))) {
                        continue;
                    }
    ...
    */
    ...
    var SBChat = {
        ...
        newConversation: function (status_code, user_id = -1, message = '', attachments = [], department = null, agent_id = null, onSuccess = false) {
            if (activeUser()) {
                SBF.ajax({
                    ...
                    department: main_chating_department,//SBF.null(department) ? this.default_department : department,
                    agent_id: this_site_aggent_id,//SBF.null(agent_id) ? this.default_agent : agent_id,
        ...
        getDepartmentCode(main_chating_department, onSuccess) {
            if (this.departments) {
                if (main_chating_department == 'all') {
                    let code = '';
                    ...
                    onSuccess(`<div data-color="${this.departments[main_chating_department].color}">${this.departments[main_chating_department].image ? `<img loading="lazy" src="${this.departments[main_chating_department].image}" />` : ''}<div>${this.departments[main_chating_department].name}<div></div>`);
                }
            } else {
                SBF.ajax({
                    ...
                    if (response) {
                        ...
                        this.getDepartmentCode(main_chating_department, onSuccess);
                    }
                });
            }
        },
        ...
        addUserAndLogin: function (onSuccess = false, lead = false) {
            ...
            settings.first_name = login_user_firstname;
            settings.last_name = login_user_lastname;
            settings.email = login_user_email;
            settings.phone = login_user_phone;
            settings.department = main_chating_department;
            SBF.ajax({
                ...
                switch (type) {
                    case 'email':
                        ...
                        if (settings.first_name) {
                            ...
                            user_settings.department = main_chating_department;
